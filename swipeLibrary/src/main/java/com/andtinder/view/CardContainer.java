...
public class CardContainer extends AdapterView<ListAdapter> {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (mOrientation == Orientation.Disordered) {
      ...
      childWidth = (int) ((R1 * Math.cos(DISORDERED_MAX_ROTATION_RADIANS) - R2 * Math.sin(
          DISORDERED_MAX_ROTATION_RADIANS)) / Math.cos(2 * DISORDERED_MAX_ROTATION_RADIANS));
      childHeight = (int) ((R2 * Math.cos(DISORDERED_MAX_ROTATION_RADIANS) - R1 * Math.sin(
          DISORDERED_MAX_ROTATION_RADIANS)) / Math.cos(2 * DISORDERED_MAX_ROTATION_RADIANS));
    } else {
      ...
      case MotionEvent.ACTION_CANCEL:
        ...
        ValueAnimator animator = ObjectAnimator.ofPropertyValuesHolder(mTopCard,
            ...
            PropertyValuesHolder.ofFloat("rotation",
                (float) Math.toDegrees(mRandom.nextGaussian() * DISORDERED_MAX_ROTATION_RADIANS)),
  ...
  private class GestureListener extends SimpleOnGestureListener {
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
      Log.d("Monica", "Fling with " + velocityX + ", " + velocityY);
      ...
      if (Math.abs(dx) > mTouchSlop &&
        ...
        Log.d("Monica", "entered in the dx or sx condition");
        ...
        return true;
      } else if (Math.abs(dy) > mTouchSlop && Math.abs(velocityY) > Math.abs(velocityX)) {
        Log.d("Monica", "entered in the move down condition");
        float targetX = topCard.getX();
        float targetY = topCard.getY();
        long duration = 0;
        ...
        if (targetY > 0) {
          Log.d("Monica", "simpleOnGesture - target y <0");
          cardModel.getOnCardDimissedListener().onAddToBasket();
        }
    ...
    private void animateCard(float velocityX, final View topCard, float targetX, float targetY,
        long duration) {
      topCard.animate()
